...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s %(levelname)-8s %(name)s: %(message)s", handlers=[logging.StreamHandler()]
)
    ...
    if RTMP_ENABLED and RTMP_URL is not None:
        try:
            ...
            if len(parts) >= 3:
                ...
                data = {"text": text, "lang": language, "trackid": track_id, "streamname": stream_name}
...
async def transcribe_chunk(
    args: argparse.Namespace, model: WhisperModel, chunk_dir: str, segment_uri: str, chunk_name: str
) -> tuple[str, str]:
        ...
        if args.both_tracks:
            ...
            if segments_orig or segments_trans:
                ...
                if segments_orig:
                    ...
                    ffmpeg_cmd.extend(
                        ["-metadata:s:s:" + str(subtitle_stream_idx), f"title=Original ({orig_lang.upper()})"]
                    )
...
async def main():
    ...
    parser.add_argument("-p", "--bind-port", type=int, help="The port to bind to (defaults to 8000)", default=8000)
    parser.add_argument(
        "-pl",
        "--public-address",
        type=str,
    ...
    parser.add_argument(
        "-t", "--transcribe", action="store_true", help="If set, transcribes rather than translates the given stream."
    )
    parser.add_argument(
        "-vf",
        "--vad-filter",
    ...
    parser.add_argument(
        "-bt", "--both-tracks", action="store_true", help="Enable both transcription and translation tracks"
    )
    parser.add_argument(
        "-f",
        "--filter-file",
    ...
    # RTMP arguments
    parser.add_argument(
        "-rtmp", "--rtmp-url", type=str, help="RTMP URL to publish captions to (e.g., rtmp://server/app/stream)"
    )
    parser.add_argument(
        "-rtmp-lang",
        "--rtmp-language",
    ...
    http_thread = threading.Thread(
        target=http_listener, daemon=True, args=((args.bind_address, args.bind_port), stop_event)
    )
    http_thread.start()
