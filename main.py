...
from typing import Any, Iterable, Optional, Protocol, Tuple, cast
...
class HTTPHandler(BaseHTTPRequestHandler):
    ...
    def _handle_request(self, send_body: bool = True) -> None:
        ...
        if self.path == "/playlist.m3u8":
            ...
            vtt_content = (
                chunk_to_vtt.get(self.path) or chunk_to_vtt_trans.get(self.path) or chunk_to_vtt_orig.get(self.path)
            )
...
def should_filter_segment(text: str) -> bool:
    ...
    if not isinstance(raw_filter_words, list):
        return False
    filter_words = [word for word in cast(list[object], raw_filter_words) if isinstance(word, str)]
...
async def transcribe_chunk(
    ...
    if args.embedded_subs:
        ...
        if args.both_tracks:
            logger.info(f"Both tracks mode: orig={len(segments_orig)}, trans={len(segments_trans)}")
            ...
            if segments:
                async with aiofiles.tempfile.NamedTemporaryFile(dir=chunk_dir, delete=False, suffix=".srt") as srt_file:
                    ...
                    if args.debug:
                        logger.debug(f"🔧 Full single track FFmpeg command: {' '.join(ffmpeg_cmd_embedded)}")
                    else:
                        logger.info(f"🔧 Executing single track FFmpeg: {' '.join(ffmpeg_cmd_embedded[:8])}...")
                        ...
                        if args.mp4_container:
                            if not init_segment_created:
                                try:
                                    ...
                                    if args.debug:
                                        logger.debug(f"Generating init.mp4: {' '.join(init_cmd_embedded)}")
                                    init_proc = await asyncio.create_subprocess_exec(*init_cmd_embedded)
                                    await init_proc.communicate()
                                    if init_proc.returncode == 0 and init_segment_path:
                                        if os.path.exists(init_segment_path):
                                            init_segment_created = True
...
async def main():
    ...
    if base_playlist.playlists:
        ...
        stream_uri = cast(Optional[str], highest_bitrate_stream.absolute_uri) or args.url
    else:
    ...
    if args.mp4_container:
        modified_base_playlist.version = str(max(7, parse_m3u8_version(modified_base_playlist.version)))
    else:
        # Keep at least version 5 for subtitle support in TS mode
        modified_base_playlist.version = str(max(5, parse_m3u8_version(modified_base_playlist.version)))
    ...
    async with aiofiles.tempfile.TemporaryDirectory() as chunk_dir:
        ...
        def schedule_cleanup() -> None:
            ...
            if cleanup_task is not None and not cleanup_task.done():
                return
            cleanup_task = asyncio.create_task(cleanup(session, chunk_dir, prev_cwd, stop_event))
        ...
        try:
            while not stop_event.is_set():
                ...
                current_segments = [
                    normalise_chunk_uri(ensure_segment_uri(segment.uri)) for segment in chunk_list.segments
                ]
                ...
                for i, chunk_name in enumerate(
                    await asyncio.gather(
                        *[
                            download_chunk(session, chunk_list.base_uri, segment, chunk_dir)
                ...
                for segment_uri, chunk_name in await asyncio.gather(
                    *[
                        transcribe_chunk(args, model, chunk_dir, segment_uri, chunk_name)
                ...
                if args.mp4_container:
                    chunk_list.version = str(max(7, parse_m3u8_version(chunk_list.version)))
                else:
                    # HLS with WebVTT requires at least version 4
                    chunk_list.version = str(max(4, parse_m3u8_version(chunk_list.version)))
                ...
                if not args.hard_subs and not args.embedded_subs:
                    ...
                    if args.both_tracks:
                        ...
                        for segment in trans_chunk_list.segments:
                            segment_uri = ensure_segment_uri(segment.uri)
                            subtitle_name = os.path.splitext(segment_uri)[0] + ".trans.vtt"
                            segment.uri = subtitle_name

                        for segment in orig_chunk_list.segments:
                            segment_uri = ensure_segment_uri(segment.uri)
                            subtitle_name = os.path.splitext(segment_uri)[0] + ".orig.vtt"
                            segment.uri = subtitle_name
