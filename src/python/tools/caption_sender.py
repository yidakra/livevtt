...
def send_caption(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"❌ Failed to send caption: {response.status_code} - {response.text}"
            )
            return False
...
def main():
    ...
    parser.add_argument(
        "--server", "-s", default="localhost", help="Wowza server hostname/IP"
    )
    parser.add_argument("--port", "-p", type=int, default=8086, help="Wowza HTTP port")
    parser.add_argument(
        "--stream", required=True, help="Stream name to send captions to"
    )
    parser.add_argument(
        "--text", "-t", default="This is a test caption", help="Caption text to send"
    )
    ...
    parser.add_argument(
        "--count", "-c", type=int, default=1, help="Number of captions to send"
    )
    parser.add_argument(
        "--interval",
        "-i",
        type=float,
        default=1.0,
        help="Interval between captions in seconds",
    )

    args = parser.parse_args()

    logger.info(
        f"Testing caption delivery to {args.server}:{args.port} for stream '{args.stream}'"
    )
    logger.info(
        "💡 To test with live streams, first publish RTMP: ffmpeg -re -i video.mp4 -c copy -f flv rtmp://localhost:1935/live/yourStreamName"
    )
    ...
    logger.info(
        f"Test complete. Successfully sent {success_count}/{args.count} captions."
    )
