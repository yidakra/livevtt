#!/usr/bin/env python3
"""OpenAI VTT Translation Utility for LiveVTT.

Scans a directory for Russian WebVTT files (*.ru.vtt), translates the text content
using OpenAI API (GPT-4, GPT-3.5-turbo, etc.), and generates English VTT files
(*.openai.en.vtt) with preserved timestamps.

This tool allows for quality comparison between Whisper, NLLB, LibreTranslate,
Mistral, and OpenAI translations without modifying the original workflow.
"""
...
LOGGER = logging.getLogger("openai_vtt_translator")
...
def translate_vtt_file(
    ...
    try:
        ...
        # Write output file
        job.openai_en_vtt_path.parent.mkdir(parents=True, exist_ok=True)
        atomic_write(job.openai_en_vtt_path, en_vtt_content)
        ...
        return {
            "status": "success",
            "ru_vtt": str(job.ru_vtt_path),
            "openai_en_vtt": str(job.openai_en_vtt_path),
...
def discover_vtt_files(
    ...
    for ru_vtt_path in input_root.rglob("*.ru.vtt"):
        # Generate output path: replace .ru.vtt with .openai.en.vtt
        openai_en_vtt_path = ru_vtt_path.with_name(ru_vtt_path.name.replace(".ru.vtt", ".openai.en.vtt"))

        # Skip if already exists (unless force)
        if openai_en_vtt_path.exists() and not force:
        ...
        jobs.append(
            TranslationJob(
                ...
                openai_en_vtt_path=openai_en_vtt_path,
            )
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Translate Russian VTT files to English using OpenAI API")
    parser.add_argument(
        "input_root",
        type=Path,
    ...
    parser.add_argument(
        "--api-url",
        type=str,
        default=os.getenv("OPENAI_API_URL", DEFAULT_OPENAI_API),
        help=f"OpenAI API endpoint URL (default: env OPENAI_API_URL or {DEFAULT_OPENAI_API})",
    )
    parser.add_argument(
        "--api-key",
        type=str,
        default=os.getenv("OPENAI_API_KEY"),
        help="API key for OpenAI (default: env OPENAI_API_KEY, required)",
    )
    parser.add_argument(
        "--model",
        type=str,
        default=os.getenv("OPENAI_MODEL", "gpt-4o"),
        help="Model name (default: env OPENAI_MODEL or gpt-4o)",
    )
    parser.add_argument(
        "--system-prompt",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Delay between API requests in seconds (default: 0.5s, adjust based on your tier)",
    )
    parser.add_argument(
        "--max-files",
        type=int,
