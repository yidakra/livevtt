...
def check_basic_stream_status(host="localhost"):
    ...
    print("🔍 Checking stream status via LiveVTT Caption API")
    print("=" * 50)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("   python test_caption.py --stream testStream")
        else:
            print(
                f"❌ LiveVTT Caption API returned unexpected status: {response.status_code}"
            )
            return False
...
def check_wowza_streams(host="localhost", port=8088):
    ...
    try:
        ...
        if response.status_code != 200:
            print(
                f"❌ Cannot access Wowza REST API at {host}:{port} (Status: {response.status_code})"
            )
        ...
        if active_streams:
            ...
            print(
                "   ffmpeg -f v4l2 -i /dev/video0 -c:v libx264 -f flv rtmp://localhost:1935/live/webcam"
            )
            print("\n   # Using OBS Studio:")
...
def main():
    ...
    parser.add_argument(
        "--host", default="localhost", help="Wowza host (default: localhost)"
    )
    parser.add_argument(
        "--port", type=int, default=8088, help="Wowza Manager port (default: 8088)"
    )
