...
def check_wowza_status():
    ...
    try:
        ...
        wowza_processes = [line for line in result.stdout.split("\n") if "wowza" in line.lower() and "grep" not in line]
...
def check_module_loaded():
    ...
    try:
        ...
        response = requests.get("http://localhost:8086/livevtt/captions/status", timeout=5)
        ...
        if response.status_code == 200:
            ...
            if "application/json" in response.headers.get("content-type", ""):
                logger.info("✅ LiveVTT Caption Module loaded (404 expected without active streams)")
                return True
...
def check_memory_stability():
    ...
    if len(memory_readings) >= 3:
        ...
        logger.info(f"✅ Memory usage stable: avg={avg_memory:.1f}%, range={min_memory:.1f}%-{max_memory:.1f}%")
...
def test_caption_api():
    ...
    try:
        ...
        payload = {"text": "Integration test caption", "language": "eng", "trackId": "99", "streamname": "testStream"}
        ...
        if response.status_code == 404:
            logger.info("✅ Caption API responding correctly (404 expected without active stream)")
            return True
        elif response.status_code == 200:
            logger.info("✅ Caption API working perfectly!")
            return True
        else:
            logger.warning(f"⚠️  Unexpected caption API response: {response.status_code}")
            return False
