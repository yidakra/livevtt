                ...
                async with session.get(url, timeout=self.timeout) as response:
                    if response.status == 200:
                        try:
                            ...
                            self.results["module_version"] = data.get(
                                "version", "unknown"
                            )
                            logger.info(
                            ...
                            if "LiveVTT" in text:
                                self.results["module_installed"] = True
                                logger.info(
                                    "✓ LiveVTT Caption Module is installed (version: unknown)"
                                )
                            else:
                                logger.error(
                                    "✗ LiveVTT Caption Module response doesn't contain expected content"
                                )
                                return False
                        return True
                    else:
                    ...
                    if response.status == 200:
                        ...
                        if "application/json" in content_type:
                            ...
                            if "<Application>" in text:
                                applications = [
                                    {"name": "live"}
                                ]  # Assume 'live' application exists
                            else:
                                applications = []

                        if applications:
                            logger.info(
                                f"✓ Found {len(applications)} applications on Wowza server"
                            )
                            ...
                            for app in applications:
                                ...
                                try:
                                    async with session.get(
                                        streams_url, timeout=self.timeout
                                    ) as streams_response:
                                        if streams_response.status == 200:
                                            content_type = streams_response.headers.get(
                                                "Content-Type", ""
                                            )
                                            if "application/json" in content_type:
                                                streams_data = (
                                                    await streams_response.json()
                                                )
                                                streams = streams_data.get(
                                                    "incomingStreams", []
                                                )
                                            else:
                                                # If it's XML, parse it differently (simple approach)
                                            ...
                                            for stream in streams:
                                                ...
                                                logger.info(
                                                    f"  - Stream: {app_name}/{stream_name}"
                                                )
                                except Exception as e:
                                    logger.warning(
                                        f"  Could not get streams for application {app_name}: {e}"
                                    )
                        else:
                            logger.warning("✗ No applications found on Wowza server")
                    else:
                        logger.warning(
                            f"✗ Could not retrieve applications (status: {response.status})"
                        )
        except Exception as e:
        ...
        try:
            ...
            async with aiohttp.ClientSession(auth=auth) as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        logger.info(
                            f"✓ Successfully sent test caption to stream {app_name}/{stream_name}"
                        )
                        return True
                    else:
                        try:
                            ...
                            logger.error(
                                f"✗ Failed to send test caption (status: {response.status}): {response_text}"
                            )
                        except Exception:
                            logger.error(
                                f"✗ Failed to send test caption (status: {response.status})"
                            )
                        return False
        except Exception as e:
...
async def main():
    parser = argparse.ArgumentParser(
        description="Check Wowza Streaming Engine configuration for LiveVTT"
    )
    parser.add_argument(
        "--url",
        "-u",
        required=True,
        help="Wowza Streaming Engine HTTP URL (e.g., http://server:8087)",
    )
    parser.add_argument(
    ...
    print(
        f"LiveVTT Caption Module installed: {'✓' if results['module_installed'] else '✗'}"
    )
