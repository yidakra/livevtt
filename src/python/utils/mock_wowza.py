...
async def handle_status(request):
    ...
    return web.json_response(
        {"module": "LiveVTTCaptionModule", "version": "1.0.0", "status": "running", "captionsReceived": len(captions)}
    )
...
async def handle_captions(request):
    ...
    try:
        ...
        return web.json_response(
            {"status": "success", "message": "Caption received and processed", "captionId": len(captions)}
        )
...
async def handle_list_captions(request):
    ...
    return web.json_response({"captions": captions, "count": len(captions)})
    ...
    return web.json_response(
        {
            ...
            "endpoints": ["/livevtt/captions/status", "/livevtt/captions", "/livevtt/captions/list"],
        }
    )
