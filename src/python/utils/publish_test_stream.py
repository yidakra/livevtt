...
def publish_stream(rtmp_url, video_file=None, duration=60):
    ...
    if not video_file:
        ...
        cmd = [
            ...
            '-i', 'testsrc=size=640x480:rate=30',  # Test source pattern
            '-f', 'lavfi',             # Use lavfi for audio too
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            print(f"Error publishing stream. FFmpeg exited with code {process.returncode}")
...
def main():
    ...
    parser.add_argument("--url", "-u", required=True, help="RTMP URL (e.g., rtmp://localhost/live/myStream)")
